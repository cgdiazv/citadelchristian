...
export async function POST(req: Request) {
  try {
    ...
    if (!apiKey) {
      ...
      return NextResponse.json({
        success: true,
        downloadUrl: "/files/Rising-1st-Grade-Summer-Plan.pdf",
      });
    }
    ...
    const { data, error } = await resend.emails.send({
      ...
      replyTo: email,
      subject: `Early Readiness Checklist Request: ${firstName} ${lastName}`,
      html: `
        <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #eee; border-radius: 8px;">
          <div style="background-color: #581076; padding: 15px 20px; border-radius: 6px; color: #ffffff; text-align: center; margin-bottom: 20px;">
            <h2 style="margin: 0; font-size: 20px;">Early Education Readiness Lead</h2>
            <p style="margin: 5px 0 0; font-size: 13px; color: #e9d5ff;">Citadel Christian School Free Resources</p>
          </div>
          ...
          <p style="margin-top: 25px; font-size: 12px; color: #888; text-align: center;">
            This email was sent automatically from the Early Education Readiness Checklist landing page.
          </p>
        </div>
      `,
    });
    ...
    console.error("Early readiness handler exception:", err);
    return NextResponse.json(
